...
// MakeTransaction implements MoneyTransctionServer.MakeTransaction
func (s *server) MakeTransaction(in *pb.TransactionRequest, stream pb.MoneyTransaction_MakeTransactionServer) error{
	...
	for i := 0; i < noOfSteps; i++{
		...
		// Usually this will be daving maoney transfer details in Db or
		...
		if err := stream.Send(&pb.TransactionResponse{
			Status: "good",
			Step: int32(i),
			Description: fmt.Sprintf("Descripition of step %d", int32(i)),
		}); err != nil {
